...
cars_csv_text = File.read(Rails.root.join('lib', 'seeds', 'cars.csv'))
cars_csv      = CSV.parse(cars_csv_text, :headers => true, :encoding => 'ISO-8859-1')

cars_csv.each do |row|
  m = CarMake.find_or_create_by(name: row['Make'])
  puts "#{m.name} car created"
end
